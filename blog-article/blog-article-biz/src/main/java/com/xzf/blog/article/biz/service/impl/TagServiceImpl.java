...
public class TagServiceImpl extends ServiceImpl<TagMapper, TagDO> implements TagService {
    ...
    public Response deleteTag(DeleteTagReqVO deleteTagReqVO) {
        ...
        ArticleTagDO articleTagRelDO = articleTagRelMapper.selectOneByTagId(tagId);

        if (Objects.nonNull(articleTagRelDO)) {
            log.warn("==> 此标签下包含文章，无法删除，tagId: {}", tagId);
            throw new BizException(BizResponseCodeEnum.TAG_CAN_NOT_DELETE);
        }

        // 根据标签 ID 删除
        int count = tagMapper.deleteById(tagId);

        return count == 1 ? Response.success() : Response.fail(BizResponseCodeEnum.TAG_NOT_EXISTED);
    }
